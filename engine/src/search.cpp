...
int history_bonus(int depth, const SearchTune& tune) {
  return std::min(tune.hist_bonus_a * depth * depth + tune.hist_bonus_b * depth, tune.hist_bonus_cap);
}

int draw_score(uint64_t) { return 0; }
...
#define NSCE_MERGE_STAT(field) field += other.field
  ...
  NSCE_MERGE_STAT(eval_abs_sum);
  NSCE_MERGE_STAT(correction_abs_sum);
  NSCE_MERGE_STAT(tt_eval_substitutions);
  NSCE_MERGE_STAT(improving_nodes);
  NSCE_MERGE_STAT(halfmove_scaled_nodes);
  NSCE_MERGE_STAT(root_moves_claimed);
...
void Search::rebuild_lmr_tables() {
  const double qdiv = std::max(1, tune_.lmr_quiet_div_x100) / 100.0;
  const double cdiv = std::max(1, tune_.lmr_capture_div_x100) / 100.0;
  for (int depth = 1; depth < 64; ++depth)
    for (int moves = 1; moves < 64; ++moves) {
      const double logs = std::log(static_cast<double>(depth)) * std::log(static_cast<double>(moves));
      lmr_quiet_[depth][moves] = static_cast<int>(0.25 + logs / qdiv);
      lmr_capture_[depth][moves] = static_cast<int>(0.10 + logs / cdiv);
    }
}

bool Search::set_tune_param(const std::string& name, int value) {
  if (!set_tune_field(tune_, name, value)) return false;
  if (name == "LmrQuietDivX100" || name == "LmrCaptureDivX100") rebuild_lmr_tables();
  return true;
}
...
void Search::clear_search_state() {
  tt_.clear();
  main_worker_ = SearchWorker{};
  for (auto& worker : helper_workers_) *worker = SearchWorker{};
  last_stats_ = {};
}

void Search::set_leaf_telemetry(const std::string& path) {
  std::lock_guard<std::mutex> lock(leaf_mutex_);
  leaf_stream_.reset();
  if (!path.empty()) {
    auto stream = std::make_unique<std::ofstream>(path, std::ios::app);
    if (*stream) leaf_stream_ = std::move(stream);
  }
}

void Search::record_leaf(const Position& pos, const char* site, int depth, int ply, int score) {
  if (!leaf_stream_) return;
  std::lock_guard<std::mutex> lock(leaf_mutex_);
  if (!leaf_stream_) return;
  *leaf_stream_ << "{\"fen\":\"" << pos.fen() << "\",\"site\":\"" << site << "\",\"depth\":" << depth
                << ",\"ply\":" << ply << ",\"in_check\":" << (pos.in_check() ? 1 : 0)
                << ",\"halfmove\":" << pos.halfmove_clock() << ",\"pieces\":" << popcount(pos.occupied())
                << ",\"score_cp\":" << score << "}\n";
  leaf_stream_->flush();
}
...
bool Search::count_node(SearchWorker& w) {
  ++w.nodes;
  if (nodes_limit_ > 0) {
    // Threads=1 (datagen, UCI default): the worker counter is the limit. A CAS
    // on the shared atomic every node was several times the increment itself.
    if (threads_ <= 1) {
      if (w.nodes >= static_cast<uint64_t>(nodes_limit_)) {
        time_expired_.store(true, std::memory_order_relaxed);
        return true;
      }
      return false;
    }
    if ((w.nodes & 15ULL) != 0) return false;
    const uint64_t total = nodes_.fetch_add(16, std::memory_order_relaxed) + 16;
    w.published_nodes = w.nodes;
    if (total >= static_cast<uint64_t>(nodes_limit_)) {
      time_expired_.store(true, std::memory_order_relaxed);
      return true;
    }
    return stopped();
  ...
  if ((w.nodes & 255ULL) != 0) return false;
  flush_nodes(w);
  if (time_up()) {
    time_expired_.store(true, std::memory_order_relaxed);
    return true;
...
void Search::flush_nodes(SearchWorker& w) {
  const uint64_t pending = w.nodes - w.published_nodes;
  if (pending == 0) return;
...
int Search::evaluate_for_search(const Position& pos) const {
  const int score = evaluate(pos);
  if (eval_scale_permille_ == 1000 || std::abs(score) >= VALUE_MATE - 256) return score;
  return score * eval_scale_permille_ / 1000;
}
...
void Search::update_quiet_stats(SearchWorker& w, const Position& pos, SearchStack* ss, Move best, const Move* quiets,
  ...
  int bonus = history_bonus(depth, tune_);
...
int Search::quiescence(Position& pos, SearchWorker& w, SearchStack* ss, int alpha, int beta, int ply) {
  ...
  if (ply >= SearchWorker::kMaxPly - 1) {
    if (in_check) {
      MoveList legal;
      generate_legal_evasions(pos, legal);
      if (legal.size == 0) return mated_in(ply);
    }
    NSCE_STAT_INC(w, evaluations);
    const int score = evaluate_for_search(pos);
    record_leaf(pos, in_check ? "q_max_check" : "q_max", 0, ply, score);
    return score;
  }
  ...
  if (!in_check) {
    ...
    if (found && std::abs(tt_score) < VALUE_MATE - 256) {
      ...
      if (stand != before_tt) NSCE_STAT_INC(w, tt_eval_substitutions);
    }
    ss->static_eval = stand;
    record_leaf(pos, "q_stand_pat", 0, ply, stand);
...
int Search::search(Position& pos, SearchWorker& w, SearchStack* ss, int depth, int alpha, int beta, int ply,
  ...
  if (ply >= SearchWorker::kMaxPly - 1) {
    ...
    NSCE_STAT_INC(w, evaluations);
    const int score = evaluate_for_search(pos);
    record_leaf(pos, in_check ? "max_check" : "max", depth, ply, score);
    return score;
  }
  ...
  if (!excluded) {
    if (!in_check && !tt_move && depth >= tune_.iir_tt_depth) {
      NSCE_STAT_INC(w, iir_reductions);
      depth -= 1;
    } else if (cut_node && depth >= tune_.iir_cut_depth) {
      NSCE_STAT_INC(w, iir_reductions);
      depth -= 1;
    }
    if (depth <= 0) return quiescence(pos, w, ss, alpha, beta, ply);
  ...
  if (in_check) {
    ...
    if (hm >= 8) {
      ...
      NSCE_STAT_INC(w, halfmove_scaled_nodes);
    }
    if (found && std::abs(tt_score) < VALUE_MATE - 256) {
      const int before_tt = eval;
      ...
      if (eval != before_tt) NSCE_STAT_INC(w, tt_eval_substitutions);
    }
  }
  ...
  if (improving) NSCE_STAT_INC(w, improving_nodes);

  if (use_razoring_ && !excluded && !pv_node && !in_check && depth <= tune_.razor_max_depth &&
      eval + tune_.razor_margin * depth < alpha) {
    NSCE_STAT_INC(w, razor_attempts);
    int ralpha = alpha - tune_.razor_margin * depth;
  ...
  if (use_rfp_ && !excluded && !pv_node && !in_check && depth <= tune_.rfp_max_depth &&
      eval - (tune_.rfp_per_depth * depth) * (improving ? 1 : 2) / 2 >= beta && eval < VALUE_MATE - 256) {
    NSCE_STAT_INC(w, rfp_attempts);
    NSCE_STAT_INC(w, rfp_cutoffs);
    return eval;
  }

  if (use_null_move_ && !excluded && !root_node && !pv_node && !in_check && !ss->skip_null &&
      depth >= tune_.nmp_min_depth && eval >= beta && non_pawn_material(pos, pos.side_to_move()) > 0) {
    NSCE_STAT_INC(w, null_attempts);
    int R = tune_.nmp_base + depth / std::max(1, tune_.nmp_depth_div) +
            std::min(tune_.nmp_eval_cap, (eval - beta) / std::max(1, tune_.nmp_eval_div));
    R = std::min(R, depth - 1);
    StateInfo st;
    pos.do_null_move(st);
    (ss + 1)->skip_null = true;
    ...
    if (score >= beta) {
      ...
      if (depth >= tune_.nmp_verify_depth) {
        ss->skip_null = true;
  ...
  if (use_probcut_ && !excluded && !root_node && !pv_node && !in_check && depth >= tune_.probcut_min_depth &&
      eval < VALUE_MATE - 256 && beta < VALUE_MATE - 256) {
    const int pc_beta = beta + tune_.probcut_margin - tune_.probcut_improving * static_cast<int>(improving);
    if (!(found && tte.bound == BOUND_UPPER && tt_score < pc_beta)) {
      ...
      const int pc_depth = std::max(1, depth - tune_.probcut_depth_sub - static_cast<int>(improving));
  ...
  for (int i = 0; i < list.size; ++i) {
    ...
    if (moving != NO_PIECE) {
      if (is_quiet) {
        // Unordered quiets: score_moves already stored history + continuation.
        hist = (scores[i] < 700000) ? scores[i]
                                    : w.history[moving][m.to()] + continuation_score(w, ss, ply, moving, m.to());
      } else if (pos.piece_on(m.to()) != NO_PIECE) {
        hist = w.capture_history[moving][m.to()][type_of(pos.piece_on(m.to()))];
      }
    }

    const int fut_margin = tune_.futility_per_depth * depth - (improving ? tune_.futility_improving : 0);
    if (use_futility_ && !root_node && !in_check && is_quiet && depth <= tune_.futility_max_depth && !pv_node &&
        eval + fut_margin <= alpha && best_score > -VALUE_MATE + 256 && hist < tune_.futility_hist) {
      NSCE_STAT_INC(w, futility_prunes);
      continue;
    }

    int lmp_limit = improving ? (tune_.lmp_improving + depth * depth) : (tune_.lmp_not_improving + depth * depth);
    if (hist > 0) lmp_limit += tune_.lmp_hist_bonus;
    ...
    if (use_lmp_ && !root_node && !in_check && is_quiet && depth <= tune_.lmp_max_depth && moves_searched >= lmp_limit &&
        best_score > -VALUE_MATE + 256 && hist < tune_.lmp_hist) {
      NSCE_STAT_INC(w, lmp_prunes);
      continue;
    }

    if (use_see_ && !root_node && !pv_node && !in_check && moves_searched > 0 && depth <= tune_.see_max_depth &&
        !m.is_promotion()) {
      const int margin = is_quiet ? -tune_.see_quiet_per_depth * depth : -tune_.see_noisy_per_depth * depth;
      NSCE_STAT_INC(w, see_prune_calls);
    ...
    int extension = 0;
    if (!root_node && !excluded && m == tt_move && depth >= tune_.se_min_depth && found &&
        tte.depth >= depth - tune_.se_tt_depth_sub &&
        (tte.bound == BOUND_LOWER || tte.bound == BOUND_EXACT) && std::abs(tt_score) < VALUE_MATE - 256) {
      NSCE_STAT_INC(w, singular_attempts);
      const int sbeta =
          tt_score - (tune_.se_margin_base + tune_.se_margin_pv * static_cast<int>(pv_node)) * depth / 64;
      ...
      if (sscore < sbeta) {
        extension = 1;
        if (pv_node && sscore < sbeta - tune_.se_double_margin * depth && ss->double_extensions < 2 &&
            ply < 2 * root_depth_) {
          extension = 2;
    ...
    const bool do_lmr = use_lmr_ && extension <= 0 && depth >= tune_.lmr_min_depth &&
                        moves_searched >= 1 + static_cast<int>(pv_node) &&
    ...
    if (do_lmr) {
      ...
      if (hist > tune_.lmr_hist_reduce) --R;
      if (hist < -tune_.lmr_hist_increase) ++R;
      if (controller_ && controller_->is_enabled()) {
  ...
  if (bound != BOUND_LOWER && best_move && !best_move.is_capture() && !best_move.is_ep()) {
    ...
    if (pc != NO_PIECE) history_update(w.history[pc][best_move.to()], history_bonus(depth, tune_));
  }
...
int Search::search_root_parallel(int depth, int alpha, int beta) {
  ...
  if (legal.size == 0) return root_.in_check() ? mated_in(0) : draw_score(0);

  // Order the root list once. Workers claim individual moves from this queue
  // instead of repeating a complete root search at different depths.
  SearchStack ordering_stack[SearchWorker::kMaxPly + SearchWorker::kStackPad + 2]{};
  int ordering_scores[MAX_MOVES]{};
  score_moves(main_worker_, root_, ordering_stack + SearchWorker::kStackPad, legal, Move{}, Move{}, 0,
              ordering_scores);
  sort_moves(legal, ordering_scores);
  ...
  auto run = [&](SearchWorker& worker) {
    Position pos = root_;
    SearchStack stack[SearchWorker::kMaxPly + SearchWorker::kStackPad + 2]{};
    SearchStack* root_ss = stack + SearchWorker::kStackPad;
    while (!stopped()) {
      const int index = next_move.fetch_add(1, std::memory_order_relaxed);
      if (index >= static_cast<int>(moves.size())) break;
      NSCE_STAT_INC(worker, root_moves_claimed);
      NSCE_STAT_INC(worker, root_moves);
      const Move move = moves[static_cast<std::size_t>(index)];
      const int local_alpha = shared_alpha.load(std::memory_order_relaxed);
      root_ss->current_move = move;
      root_ss->moved_piece = root_.piece_on(move.from());
      root_ss->excluded = {};
      pos = root_;
      StateInfo st;
      pos.do_move(move, st);
      const int child_depth = std::max(0, depth - 1);
      int score = -search(pos, worker, root_ss + 1, child_depth, -beta, -local_alpha - 1, 1, false);
      if (!stopped() && score > local_alpha && score < beta)
        score = -search(pos, worker, root_ss + 1, child_depth, -beta, -local_alpha, 1, false);
      pos.undo_move(move, st);
      if (stopped()) break;

      std::lock_guard<std::mutex> lock(best_mutex);
      if (score > best_score || !best_move) {
        best_score = score;
        best_move = move;
        best_pv[0] = move;
        best_pv_len = 1;
        const int child_pv_len = std::min(worker.pv_len[1], SearchWorker::kMaxPly - 1);
        for (int i = 0; i < child_pv_len; ++i) best_pv[i + 1] = worker.pv[1][i];
        best_pv_len += child_pv_len;
      }
  ...
  launch_helper_job([&](SearchWorker& worker) { run(worker); });
  run(main_worker_);
  wait_helper_job();
...
SearchInfo Search::go_prepared(const SearchLimits& limits) {
  ...
  main_worker_.stats = {};
  last_stats_ = {};
  // Node-limited searches (datagen) are ~5k nodes. Halving history every move
  // dominated those searches. Gravity in history_update already bounds entries.
  if (limits.nodes <= 0) {
  ...
  int prev_score = 0;
  int score_volatility = 0;
  int64_t prev_ms = 0;
  int64_t previous_iteration_ms = 0;
  ...
  for (int depth = 1; depth <= max_depth; ++depth) {
    ...
    int delta = tune_.asp_delta_base + std::min(tune_.asp_delta_vol, score_volatility / 2);
    ...
    score_volatility = std::abs(score - prev_score);
    prev_score = score;
    ...
    const int iteration_ms = static_cast<int>(ms - prev_ms);
    const int prior_iteration_ms = static_cast<int>(previous_iteration_ms);
    if (limits.depth == 0 && maximum_ms_ > 0 && best && !pondering_.load(std::memory_order_relaxed)) {
      ...
      if (iteration_ms > 0 && remaining < iteration_ms / std::max(1, tune_.tm_iter_div)) break;
    } else {
      prev_ms = ms;
    }
    ...
    if (!pondering_.load(std::memory_order_relaxed) && use_soft_time_ && optimum_ms_ > 0) {
      ...
      if (score_volatility > 80)
        soft_limit = std::min(maximum_ms_, std::max(soft_limit, 3 * optimum_ms_ / 2));
      if (prior_iteration_ms > 0 && iteration_ms > 2 * prior_iteration_ms)
        soft_limit = std::min(soft_limit, std::max(1, 3 * optimum_ms_ / 4));
  ...
  if (!silent_)
    std::cout << "info string stats qnodes_pct " << qnode_pct << " evals " << last_stats_.evaluations << " tt "
              ...
              << " futility " << last_stats_.futility_prunes << " lmp " << last_stats_.lmp_prunes
              << " eval_abs_sum " << last_stats_.eval_abs_sum << " corr_abs_sum "
              << last_stats_.correction_abs_sum << " tt_eval_subs " << last_stats_.tt_eval_substitutions
              << " improving " << last_stats_.improving_nodes << " hm_scaled "
              << last_stats_.halfmove_scaled_nodes << " root_claimed " << last_stats_.root_moves_claimed << std::endl;
#endif
